...
class ExpansiveVisualPrompt(nn.Module):
    def __init__(self, out_size, mask, init="zero", normalize=None):
        ...
        self.register_buffer(
            "mask",
            F.pad(mask, (self.l_pad, self.r_pad, self.l_pad, self.r_pad), value=1),
        )

    def forward(self, x):
        x = (
            F.pad(x, (self.l_pad, self.r_pad, self.l_pad, self.r_pad), value=0)
            + torch.sigmoid(self.program) * self.mask
        )
