...
def predictive_distribution_based_multi_label_mapping(dist_matrix, mlm_num: int):
    assert mlm_num * dist_matrix.size(1) <= dist_matrix.size(
        0
    ), "source label number not enough for mapping"
...
def generate_label_mapping_by_frequency(
    visual_prompt, network, data_loader, mapping_num=1
):
    ...
    pbar = (
        tqdm(
            ...
            desc=f"Frequency Label Mapping",
            ncols=100,
        )
    ...
    pairs = torch.nonzero(
        predictive_distribution_based_multi_label_mapping(dist_matrix, mapping_num)
    )
